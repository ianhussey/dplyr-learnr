---
title: "Data wrangling in R and dplyr"
author: Allison Horst + minor changes by Ian Hussey
output: 
  learnr::tutorial:
    css: css/style.css
...
#### dplyr-learnr creator!

Apart from the most minor reordering of the lessons above, this Shiny app was written by [Allison Horst](https://allisonhorst.com/) and all credit goes to her.
...
<body>
    <a class="button" href="https://github.com/allisonhorst/dplyr-learnr">Fork Allison's original GitHub repo
    </a>
    
<body>
    <a class="button" href="https://github.com/ianhussey/dplyr-learnr">Fork Ian's slightly modified version GitHub repo
    </a>
...
The 3 species of penguins in this dataset are Adelie, chinstrap and gentoo. They are all awesome and adorable.
...
As a reminder, here's a sample from the **penguins** data (5 observations / 344 total). See the [Meet the data](#meet-the-data) heading for more information about the dataset.
...
Rename the `year` column to `study_yr`, and `body_mass_g` to `mass`. You can do this by only using `rename()` once!

```{r, echo = TRUE, warning = FALSE, message = FALSE}
penguins %>%
  rename(study_yr = year, 
         mass = body_mass_g)
```
...
### RENAME PRACTICE EXERCISES

#### `r fa("fas fa-keyboard", fill = "purple")` Practice Activity 1

Using the `penguins` dataset, rename the `flipper_length_mm` column to `flipper_mm`.
...
Using the `penguins` dataset, rename the `island` column to `island_name` and the `species` column to `penguin_spp`.
...
- Return all the column names with `colnames()`.
- Find the column that contains the string "_g".
- Rename this column so that "_g" becomes "_grams". 
...
While making subsets of variables is rarely *necessary* for analyses (and is often unadvised), it can make large datasets with many variables more manageable.
...
There are **many** helper functions that add power to `select()`. For example, selecting all variables of a specified class, or selecting all variables that start or end with a certain string pattern. 
...
As a reminder for the following examples, here's a sample from the **penguins** data (5 observations / 344 total). See the [Meet the data](#meet-the-data) heading for more information about the dataset.
...
### GROUP_BY %>% SUMMARIZE PRACTICE EXERCISES
...
`dplyr::across()` makes it easy to apply the same transformation to multiple columns.
